...
describe CongressMember do
  ...
  describe "instance" do
    ...
    describe "Senator" do
      before do
        @senator = CongressMember.new(first_name: "first", last_name: "last", state: "DC", district: nil, party: "R", twitter_handle: "@handle", type_by_house: "Senator")
      end

      subject { @senator }
      
      describe "when there's a second senator in the same state" do
        before do
          same_state_senator = @senator.dup
          same_state_senator.save
        end 
        
        it { should be_valid }
      end

      describe "when there's already two senators in the same state" do
        before do
          same_state_senator = @senator.dup
          second_same_state_senator = @senator.dup
          same_state_senator.save
          second_same_state_senator.save
        end 
